...
{  
  ...
  # TO-DO ω withthreshold functions (more parameters)
...
phasePlot <- function(FP, To, P, eps, lam, oma, h, gam, s) 
{
  FP.flowField  <- 
    flowField(FP, x.lim = c(0.01, 99.9), y.lim = c(0.01, 99.9),
              ...
              xlab="F", ylab="M",
              main=paste0("h = ", h), #other symbols λ ω
...
quartz("phase portraits endogenous ω (logistic)", width=7, height=7)
par(mfrow=c(2,2))
...
for(a in c(0.5,1,2.5,4)) {
  phasePlot(FP, To, P, eps, lam, oma, h=0.5, gam, s)
}
mtext("ω=(a/P)*M*(1-(M/P))", cex=0.8, line=0.5, adj=-0) #other symbols λ ω
quartz.save("PP endogenous ω (logistic).pdf", type = "pdf", dpi = 500)
