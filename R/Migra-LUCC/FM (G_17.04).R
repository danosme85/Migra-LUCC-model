...
# could seperate functions and phase plots into seperate files in future
...
# function including changes in ecological regrowth based on forest cover
...
{  
  ...
  #10. seedSat					Phi				dimensionless 		
	...
	LabCost					<-parameters[8]	
	CvsL						<-parameters[9]	
	seedSat					<-parameters[10]	

	# Forest/Agriculture Dynamics
	boughtlabour <- CvsL * LabCost * prRemit * urbanUtil * M
	fForestGrowth <- forestGrowth * (1-exp(-seedSat*F))		# limited growth at low levels of F

  	dy[1] <- fForestGrowth * F *(1-F) - forestClearingRate*(1-M + boughtlabour)*F #Forest
...
phasePlotFM <-function(FM, tSteps, ptOffSet, forestGrowth,forestClearingRate,migrationDecisionRate,decisionSd,urbanUtil,agUtil,prRemit,LabCost,CvsL,seedSat) 
{
  ...
  startPts <- matrix(c(ptOffSet, ptOffSet, ptOffSet, hiPtOffset, hiPtOffset, ptOffSet, hiPtOffset, hiPtOffset), ncol = 2, nrow = 4, byrow = TRUE)
	...
	parametersFM <-c(forestGrowth,forestClearingRate,migrationDecisionRate,decisionSd,urbanUtil,agUtil,prRemit,LabCost,CvsL,seedSat)	
